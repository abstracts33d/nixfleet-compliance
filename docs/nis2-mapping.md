...
- **Directive:** Directive (EU) 2022/2555 of the European Parliament and of the Council of 14 December 2022 on measures for a high common level of cybersecurity across the Union (NIS 2 Directive)
- **Published:** 27 December 2022 (Official Journal of the European Union, L 333)
- **Full text (EUR-Lex):** <https://eur-lex.europa.eu/eli/dir/2022/2555/oj>
...
|---------|------------|----------------|-------------|----------|
| 21(a) | Risk analysis, IS security policy | `_baseline-hardening` | Kernel hardening, sysctl, service blocklist, filesystem permissions | Hardening score, deviation list |
| 21(b) | Incident handling | `_incident-response` | Alert rules, log retention, rollback readiness | Alert status, rollback generations, last test |
| 21(c) | Business continuity, backup, DR | `_backup-retention` + `_disaster-recovery` | Retention policy, restore testing, RTO measurement | Backup status, retention compliance, RTO metrics |
| 21(d) | Supply chain security | `_supply-chain` | Flake.lock pinning, SBOM generation, reproducibility attestation | Complete SBOM, build hash, input provenance |
| 21(e) | Vulnerability handling | `_vulnerability-mgmt` | CVE scanning, patch staleness, deployment gates | CVE results, nixpkgs age, time-to-patch |
...
| 21(g) | Cyber hygiene | `_baseline-hardening` | Password policy, session timeout, login banner | Included in hardening score |
| 21(h) | Cryptography/encryption | `_encryption-at-rest` + `_encryption-in-transit` | LUKS verification, swap encryption, tmpfs + TLS enforcement, cert monitoring | Partition encryption status, swap status + TLS config |
| 21(i) | Access control, asset mgmt | `_access-control` + `_asset-inventory` | SSH key-only, root disabled, sudo restrictions + host/service/network inventory | Auth method, privileged users, SSH keys + complete host inventory |
| 21(j) | MFA, secure comms | `_authentication` | PAM MFA, SSH certificates, service accounts | MFA status, cert authority, service account inventory |
...
```json
{
  ...
  "overall": "12/12 controls compliant"
}
```
