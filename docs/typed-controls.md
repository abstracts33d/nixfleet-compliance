...
Two projections let the control plane gate rollouts at CI time (static) AND let the agent surface runtime evidence (probe). Before this migration, only runtime probes existed; static gates were ad-hoc. The wire contract for the `probeDescriptor` payload lives in [`arcanesys/nixfleet` `docs/design/contracts.md` §I.3](https://github.com/arcanesys/nixfleet/blob/main/docs/design/contracts.md).
